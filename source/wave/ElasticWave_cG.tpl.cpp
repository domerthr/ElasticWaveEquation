...
template <int dim>
ElasticWave_cG<dim>::ElasticWave_cG(unsigned int s, unsigned int r)
    :   triangulation(1),
...
template <int dim>
inline dealii::SymmetricTensor<2, dim> ElasticWave_cG<dim>::get_strain(const dealii::Tensor<2,dim> &grad) {
...
template <int dim>
void ElasticWave_cG<dim>::set_input_parameters(
...
template <int dim>
void ElasticWave_cG<dim>::init_functions() {
...
template <int dim>
void ElasticWave_cG<dim>::init_grid() {
...
template <int dim>
void ElasticWave_cG<dim>::time_steps(const unsigned int refinement_cycle) {

    // This collection simplifies the time step increments
    fest::cg::TimeIteratorCollection<dim> it_coll = fest::cg::TimeIteratorCollection<dim>();
    ...
    for (; !it_coll.at_end(); it_coll.increment()) {
        ...
        fest::cg::VectorTools::extract_subvector_at_time_dof(*slab_it.solution,
                                                             slab_initial_value,
                                                             slab_it.tria->temporal()->n_global_active_cells());
        ...
        const unsigned int n_dofs_time  = slab_it.dof->n_dofs_time_test() * triangulation.M();
        refinement_table.add_value("dofs (space)", n_dofs_space);
        refinement_table.add_value("dofs (time)", n_dofs_time + 1);
...
template <int dim>
void ElasticWave_cG<dim>::setup_dofs_on_slab() {
    ...
    fest::cg::VectorTools::interpolate_boundary_values(*slab_it.dof, 0,*function.boundary_function, slab_constraints);

    slab_constraints->close();


    // Construct the space-time sparsity pattern for this slab
    dealii::DynamicSparsityPattern dsp(slab_it.dof->n_dofs_spacetime_trial());
    fest::cg::DoFTools::make_sparsity_pattern(*slab_it.dof, dsp);
    ...
    slab_it.solution->reinit(slab_it.dof->n_dofs_spacetime_trial());
    slab_system_rhs.reinit(slab_it.dof->n_dofs_spacetime_trial());
...
template <int dim>
void ElasticWave_cG<dim>::assemble_system_on_slab() {
    ...
    idealii::spacetime::QGauss<dim> quad(fe.spatial()->degree + 2, fe.temporal_trial()->degree + 2);
    fest::cg::FEValues<dim> fe_values(fe, quad,
                                      dealii::update_values | dealii::update_gradients | 
                                      dealii::update_quadrature_points | dealii::update_JxW_values);


    const unsigned int dofs_per_spacetime_cell_trial = fe.dofs_per_cell_trial;
    const unsigned int dofs_per_spacetime_cell_test = fe.dofs_per_cell_test;
    ...
    dealii::FullMatrix<double> local_matrix(N * dofs_per_spacetime_cell_trial, 
                                            N * dofs_per_spacetime_cell_trial);
    dealii::Vector<double> local_rhs(N * dofs_per_spacetime_cell_trial);
    std::vector<dealii::types::global_dof_index> local_dof_indices(N * 
                                                                   dofs_per_spacetime_cell_trial);
    ...
    for (const auto &space_cell : slab_it.dof->spatial()->active_cell_iterators()) {
        ...
        for (const auto &time_cell : slab_it.dof->temporal_trial()->active_cell_iterators()) {
            

            for (const auto &time_cell_test : slab_it.dof->temporal_test()->active_cell_iterators()) {

                if (time_cell->index() == time_cell_test->index()) {
                    n = time_cell->index();

                    // recalculate the local information of the current temporla element
                    fe_values.reinit_time(time_cell, time_cell_test);
                    ...
                    for (unsigned int q = 0; q < n_quad_spacetime; ++q) {
                        ...
                        for (unsigned int i = 0; i < dofs_per_spacetime_cell_test; ++i) {

                            local_rhs(dofs_per_spacetime_cell_test + i + n * dofs_per_spacetime_cell_trial) += 
                                (function.rhs_function->value(x_q) *                                                // f(t_qq, x_q)
                                 fe_values.vector_value_test(displacement, i, q) *                                  // φ^u_{i_time, i_space}(t_qq, x_q)
                                 fe_values.JxW(q));                                                                 // dx dt
                            

                            for (unsigned int j = 0; j < dofs_per_spacetime_cell_trial; ++j) {

                                local_matrix(dofs_per_spacetime_cell_test + i + n * dofs_per_spacetime_cell_trial,
                                             j + n * dofs_per_spacetime_cell_trial) += 
                                    
                                     (parameter_set->rho *                                              // ρ
                                      fe_values.vector_dt(velocity, j, q) *                             // ∂_t φ^v_{i_time, i_space}(t_qq, x_q)
                                      fe_values.vector_value_test(displacement, i, q)                   // φ^u_{j_time, j_space}(t_qq, x_q)
                                    ...
                                    + dealii::scalar_product(2 * parameter_set->lame_coefficient_mu *   // 2 * μ
                                      ...
                                      fe_values.vector_space_grad_test(displacement, i, q))             // ∇_x φ^u_{i_time, i_space}(t_qq, x_q)
                                    ...
                                    + fe_values.vector_dt(displacement, j, q) *                         // ∂_t φ^u_{j_time, j_space}(t_qq, x_q)                   
                                      fe_values.vector_value_test(velocity, i, q)                       // φ^v_{i_time, i_space}(t_qq, x_q)
...
template <int dim>
void ElasticWave_cG<dim>::apply_temporal_conditions()
{
    ...
    if (parameter_set->junker) {    
        ...
        apply_initial_values(0, slab_it.dof->n_dofs_space());
...
template <int dim>
void ElasticWave_cG<dim>::apply_initial_values(unsigned int start, unsigned int end) {

    // clear from start to end all rows of system matrix
    for (unsigned int i = start; i < end; ++i){
        // iterate over all entries and set A_ij = δ_ij
        for (typename dealii::SparseMatrix<double>::iterator j = slab_system_matrix.begin(i); j != slab_system_matrix.end(i); ++j) {
            j->value() = 0.;
            slab_system_matrix.set(i, i, 1.);
            slab_system_rhs(i) = slab_initial_value(i);
        }
   }
...
template <int dim>
void ElasticWave_cG<dim>::apply_final_values() {


    // computing Final Values
    dealii::Vector<double> final_value(slab_it.dof->n_dofs_space());
    dealii::VectorTools::interpolate(*(slab_it.dof->spatial()), *function.final_value_function, final_value, dealii::ComponentMask());

    // claer last N_space rows of system matrix and skip u-DoFs
    for (unsigned int i = slab_it.dof->n_dofs_spacetime_trial() - n_space_v; i < slab_it.dof->n_dofs_spacetime_trial(); ++i) {
        // iterate over all entries and set A_ij = δ_ij
        for (typename dealii::SparseMatrix<double>::iterator j = slab_system_matrix.begin(i); j != slab_system_matrix.end(i); ++j) {
            j->value() = 0.;
            slab_system_matrix.set(i, i, 1.);
            slab_system_rhs(i) = final_value(i + n_space_u + n_space_v - slab_it.dof->n_dofs_spacetime_trial());
        }
    }
...
template <int dim>
void ElasticWave_cG<dim>::solve_on_slab() {
...
template <int dim>
void ElasticWave_cG<dim>::output_results(const unsigned int refinement_cycle) {
    
    const std::string output_dir = "../output/cG/example-" + std::to_string(parameter_set->example) + "/"
    ...
    auto n_dofs = slab_it.dof->n_dofs_time_trial();

    // time
    std::vector<dealii::Point<1>> time_support_points(slab_it.dof->temporal_trial()->n_dofs());
    dealii::DoFTools::map_dofs_to_support_points(
        dealii::MappingQ1<1, 1>(), *slab_it.dof->temporal_trial(), time_support_points);

    unsigned int start_index = (slab == 0 ? 0 : n_dofs / 2);

    for (unsigned int i = start_index; i < n_dofs; i++) {
        ...
        fest::cg::VectorTools::extract_subvector_at_time_dof(*slab_it.solution, local_solution, i);
        ...
        filename << "solution(" << fe.temporal_trial()->degree << ")_t_" << slab * n_dofs / 2 + i << ".vtk";
...
template <int dim>
void ElasticWave_cG<dim>::process_solution(unsigned int cycle) {


    idealii::spacetime::QGauss<dim> quad(fe.spatial()->degree + 2, fe.temporal_trial()->degree + 2);
    L2_error += fest::cg::VectorTools::calculate_L2L2_squared_error_on_slab(*slab_it.dof, *slab_it.solution, *function.exact_solution, quad);
    ...
    if (slab == (triangulation.M() - 1)) {
        ...
        const unsigned int n_dofs_space = slab_it.dof->n_dofs_space();
        const unsigned int n_dofs_time  = slab_it.dof->n_dofs_time_test() * triangulation.M();
        const unsigned int n_dofs = n_dofs_space * (n_dofs_time + 1);
        ...
        convergence_table.add_value("dofs(time)", n_dofs_time + 1);
...
template <int dim>
void ElasticWave_cG<dim>::plot_solution(const unsigned int refinement_cycle) {
...
template <int dim>
void ElasticWave_cG<dim>::print_convergence_table() {
    ...
    // if (false && parameter_set->refine_time)
...
void ElasticWave_cG<dim>::run() {
    ...
    for (unsigned int cycle = 0; cycle < parameter_set->max_refinements_time; ++cycle) {
        ...
        refinement_table.add_value("total", 1);
...
#include "ElasticWave_cG.inst.in"